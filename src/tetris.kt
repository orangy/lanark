...
class SDL_Visualizer(val width: Int, val height: Int): GameFieldVisualizer, UserInput {
    ...
    init {
        if (SDL_Init(SDL_INIT_EVERYTHING) != 0) {
            println("SDL_Init Error: ${getSDLErrorText()}")
            throw Error()
        }
        ...
        memScoped {
            ...
            if (SDL_GetCurrentDisplayMode(0, displayMode.ptr.reinterpret()) != 0) {
                println("SDL_GetCurrentDisplayMode Error: ${getSDLErrorText()}")
                SDL_Quit()
                throw Error()
            }
        ...
        if (window == null) {
            println("SDL_CreateWindow Error: ${getSDLErrorText()}")
            SDL_Quit()
            throw Error()
        }
        this.window = window
        ...
        if (renderer == null) {
            ...
            println("SDL_CreateRenderer Error: ${getSDLErrorText()}")
            SDL_Quit()
            throw Error()
        }
    ...
    private fun loadImage(win: CPointer<SDL_Window>, ren: CPointer<SDL_Renderer>, imagePath: String): CPointer<SDL_Texture> {
        ...
        if (bmp == null) {
            ...
            println("SDL_LoadBMP_RW Error: ${getSDLErrorText()}")
            SDL_Quit()
            throw Error()
        }
        ...
        if (tex == null) {
            SDL_DestroyRenderer(ren)
            SDL_DestroyWindow(win)
            println("SDL_CreateTextureFromSurface Error: ${getSDLErrorText()}")
            SDL_Quit()
            throw Error()
        }
        return tex
